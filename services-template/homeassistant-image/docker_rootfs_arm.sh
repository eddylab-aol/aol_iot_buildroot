...
WORK_DIR="$(cd $(realpath -s $dir)/../ ; pwd -P)"
ROOTFS_NAME=$(basename $dir)
ROOTFS_DIR=$WORK_DIR/$ROOTFS_NAME
PULL_DIR=$WORK_DIR/$dir
TMP_DIR=$WORK_DIR/_temp

echo "$WORK_DIR $ROOTFS_NAME $ROOTFS_DIR $PULL_DIR $TMP_DIR"

mv $PULL_DIR $TMP_DIR
mkdir $ROOTFS_DIR

find $TMP_DIR -type f -iname "*.tar" -print -execdir tar xf {} -C $ROOTFS_DIR \;
cd $WORK_DIR
tar czf $ROOTFS_NAME.tar.gz $ROOTFS_NAME/*
rm -rf $TMP_DIR
rm -rf $ROOTFS_DIR

echo "Finished make $ROOTFS_NAME.tar.gz"
